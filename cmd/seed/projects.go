...
func upsertProject(tx *gorm.DB, project projectSeed, adminID uuid.UUID) error {
	...
	const memberSQL = `INSERT INTO project_members(project_id,user_id,role)
		VALUES(?,?,'owner') ON CONFLICT(project_id,user_id)
		DO UPDATE SET role='owner',updated_at=now()`
	if err := tx.Exec(memberSQL, project.id, adminID).Error; err != nil {
