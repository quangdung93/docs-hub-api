...
// Integration test của document repository cần schema THẬT: quan hệ
// documents <- document_revisions với cột deleted_at là chính thứ đang kiểm.
// Dựng bằng vài câu CREATE TABLE tối giản thì test mất ý nghĩa — nó phải chạy
// trên đúng schema mà production dùng.
//
// Khung này sao lại từ internal/module/ingestion/testdb_integration_test.go.
// Cố ý sao chứ không tách thành package dùng chung: hai bộ test cần database
// RIÊNG với tên khác nhau, và một package test helper dùng chung sẽ kéo theo
// phụ thuộc ngược từ module này sang module kia.
//
...
const (
	testDatabase  = "document_hub_document_it"
