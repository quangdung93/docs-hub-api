...
Toàn bộ vai trò trên là **role trong `project_members`** — riêng của từng dự án, khác với role hệ thống (admin/user) trong JWT. Ở mọi route có cột "Quyền", **admin hệ thống luôn bypass** (xem mục RBAC).
...
- **Nghiệp vụ** (HTTP 200, `success:false`): `ALREADY_MEMBER` (mời user đã là thành viên/đang chờ xác nhận), `INVITE_NOT_PENDING` (accept một lời mời không còn ở trạng thái pending), `CANNOT_MODIFY_OWNER` (đổi role/gỡ owner qua API quản lý thành viên), `CONFIRM_NAME_MISMATCH` (xóa dự án nhưng `confirm_name` không khớp tên hiện tại).
...
- `usecase/project_uc_test.go` — toàn bộ service method, gồm cả `ensureNotOwner` (chặn đổi role/gỡ owner) và `Delete` xác nhận tên (`ErrConfirmNameMismatch`); mock qua `domain/mocks` + `common/port/mocks` (sinh bằng mockery, `make mocks`).
- `delivery/http/project_handler_test.go` — cả 9 API qua router thật (bao gồm middleware RBAC), assert đúng envelope ISC; có test riêng cho Editor bị chặn `PATCH /projects/:id`, thiếu/sai `confirm_name` khi xóa.
