...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/internal/api/v1/projects/{id}/documents/uat-report": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                ...
                "description": "Điền tài liệu trong phạm vi đã chọn (version, change request,\nhoặc để trống cả hai = toàn dự án) vào template UAT Report chuẩn ISC.\nChỉ thành viên quyền editor trở lên mới xuất được. format: \"xlsx\" (mặc định) hoặc \"pdf\".",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "parameters": [
                    ...
                    {
                        "description": "PO/PM/phạm vi/định dạng UAT — có thể bỏ trống toàn bộ",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/http.UATReportRequest"
                        }
                    }
                ],
                "responses": {
        ...
        "/internal/api/v1/projects/{id}/documents/{document_id}/urd/analyses/{analysis_id}/resolutions": {
            "post": {
                ...
                "description": "Khi resolved_cases đạt total_cases sau lời gọi này, hệ thống tự động\nmerge nội dung các case được đưa vào tài liệu vào cuối file .docx gốc\nvà tạo revision mới. Case không gửi include_in_document mà có hướng\ngiải quyết dạng \"không áp dụng\" sẽ TỰ ĐỘNG bị loại khỏi tài liệu.\nTài liệu không phải .docx, hoặc không còn case nào được đưa vào tài\nliệu, vẫn hoàn tất bình thường nhưng KHÔNG sinh revision mới — xem\ncờ new_revision_created trong data.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
